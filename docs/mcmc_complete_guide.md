# MCMCとは何か - 複雑な確率分布のサンプリング手法

## 目次

1. [MCMCの本質と重要性](#第1章-mcmcの本質と重要性)
...
従来の解析的手法では、このような分布から直接サンプルを得ることは困難です。そこで有効なのが**マルコフ連鎖モンテカルロ法（MCMC）**です。

MCMCは、「直接サンプリングできない複雑な分布を、間接的にマルコフ連鎖のシミュレーションによって扱う」という手法です。この手法により、従来は計算が困難だった複雑な確率モデルを、実際の計算機上で扱うことが可能になりました。
...
MCMCは科学技術に大きな影響を与えています：

1. **理論と実践の橋渡し**: 数学的に精密だが計算困難なモデルを実用化
2. **不確実性の定量化**: 点推定だけでなく、予測の信頼区間も提供
3. **複雑系の理解**: 高次元・非線形・多峰性を持つ系の解析を可能に
4. **学際的応用**: 物理学、生物学、経済学、工学など分野横断的な手法として普及

現在、MCMCは現代的なデータサイエンスや科学計算において重要な役割を果たしています。この手法は、21世紀の「計算による科学」を支える基盤技術の一つとなっています。

次章では、この手法の数学的基礎を詳しく見ていきます。
...
マルコフ連鎖が定常分布に収束するためには、**エルゴード性**が必要です。エルゴード性は主に以下の条件から構成されます：
...
提案分布の標準偏差は、サンプリング効率に大きく影響します。適切な値を見つけるためのチューニング例を見てみましょう。
